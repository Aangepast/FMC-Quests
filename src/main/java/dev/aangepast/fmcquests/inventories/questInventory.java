...
public class questInventory {
    ...
    public void openQuestInventory(Player player, User user){
        ...
        ItemStack shop = new ItemBuilder(Material.PAPER, 1).setName("SHOP?").toItemStack();
        ...
        ItemStack playerInfo = new ItemBuilder(Material.PLAYER_HEAD, 1).setName(ChatColor.WHITE + ChatColor.BOLD.toString() + "Jouw statistieken:").addLoreLine(ChatColor.WHITE + "Quests voltooid: " + ChatColor.GREEN + user.getCompleted()).addLoreLine(ChatColor.WHITE + "Actieve quests: " + ChatColor.RED + user.getQuests().size()).addLoreLine(ChatColor.WHITE + "Orbs tegoed: " + ChatColor.DARK_PURPLE + user.getOrbsToReceive()).addLoreLine(ChatColor.WHITE + "Netwerk XP tegoed: " + ChatColor.LIGHT_PURPLE + user.getXpToReceive()).addLoreLine(ChatColor.WHITE + "Kindomspelen punten: " + ChatColor.AQUA + user.getPoints()).addLoreLine(" ").addLoreLine(ChatColor.GRAY + ChatColor.ITALIC.toString() + "Dit zijn jouw meest recente statistieken.").setSkullOwner(player.getName()).toItemStack();
